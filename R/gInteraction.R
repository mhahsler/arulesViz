...
gInteraction <- function(buttons = data.frame(),
  buttonCol = NULL) {
  if (is.null(buttonCol))
    buttonCol <- c("lightblue", "orange")
  
  #do some checks
  if (!is(buttons, "data.frame"))
    stop("gInteraction expects a data.frame to specify 'buttons'!")
  if (nrow(buttons) > 0) {
    if (any(is.na(match(
      c("active", "y", "x", "w", "h"),
      names(buttons)
    ...
    for (f in c("x", "y", "w", "h"))
      if (!is(buttons[, f], "unit"))
        buttons[, f] <- I(unit(buttons[, f], "npc"))
  }
...
drawButtons <- function(gI, which = NULL) {
  if (nrow(gI$buttons) < 1)
    return(invisible())
  
  if (!is.null(which))
    gI$buttons <- gI$buttons[which, ]
...
lastButton <- function(gI, activeOnly = TRUE) {
  if (activeOnly && !isActiveButton(gI, gI$lastButton))
    return(NULL)
  else
    return(gI$lastButton)
}

selection <- function(gI)
  gI$sel
...
isActiveButton <- function(gI, which = NULL) {
  ...
  if (is.null(which))
    which <- rownames(gI$buttons)
  structure(gI$buttons[which, ]$active, names = which)
}

changeButton <- function(gI,
  which = NULL,
  to = NULL,
  redraw = TRUE) {
  ## fixme: check if button exists
  ...
  if (is.null(which))
    which <- rownames(gI$buttons)
  if (is.null(to))
    to <- !gI$buttons[which, ]$active
  gI$buttons[which, ]$active <- to
  if (redraw)
    drawButtons(gI, which)
  gI
}
