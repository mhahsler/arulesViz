...
plot(x, method = NULL, measure = "support", shading = "lift",
     engine = "default", data = NULL, control = NULL, ...)
@

\begin{itemize}
    \item \code{x} is the set of rules to be visualized,
    \item \code{method} is the visualization method (some are given in Table~\ref{tab:methods}),
    \item \code{measure} and \code{shading} contain the interest measures
used by the plot,
\item \code{engine} was introduces in a recent release to let the user
choose between different rendering engines (e.g., using R's grid static graphics, interactive features or HTML widgets),
\item \code{data} can contain the transaction data set used to mine the
rules (only necessary for some methods),
\item \code{control} is a list
with visualization-specific control arguments to customize the plot. Using
        the the control argument \code{verbose = TRUE} will show all
        available arguments and the default values for the chosen method and
        engine.
\item \code{...} additional arguments are for convenience added to the control list.
\end{itemize}
...
is activated using either \code{engine = "ineractive"}
or \code{engine = "htmlwidget}".
...
The visual impression is improved by
reordering rows and columns in the matrix
such that rules with higher lift values (measure used for shading)
are placed closer to the left-top corner.
...
3D bars (engine ``3d'') instead of colored rectangles.
...
If we specify a vector with two measures for dhading, both measures
...
Figure~\ref{fig:matrix_col1} shows the plot with two
...
plot(rules, method="grouped matrix", k=50)
@

Note that \code{k} is automatically added to the control list.
